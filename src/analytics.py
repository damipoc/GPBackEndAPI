...
def aggregate(data, interval=5, mode=None, key=None):
    ...
    while data and (data[-1]["ts"] - start_time).total_seconds() >= interval:
        aggregate = {}
        aggregate["_id"] = data[0]["_id"]
        ...
        for d in data:
            ...
            if (d["ts"] - start_time).total_seconds() > interval:
                break
...
def anomaly_filter(stds, data, key):
    """data_filter wrapper for anomaly detection.

    Gets all elements of a list of dicts 'data' for which key 'key' is more than
    'stds' standard deviations away from the mean value of 'key' for all values
    in 'data'.
    ...
    Returns:
        A list of dicts containing anomalous data. Datapoints in 'data' which
        are within 'stds' standard deviations from the mean or ommitted.
    """
