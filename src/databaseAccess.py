...
class DatabaseAccess():
    ...
    def __init__(self):
        _mongo_server = {"host":"localhost",
                         "port":27017}

        self._mongo_object = pymongo.MongoClient(_mongo_server["host"], _mongo_server["port"])

        self._mongo_devices = self._mongo_object["devices"]

        self._sql_object = mysql.connector.connect(host="localhost",
                                                    user="dbaccess",
                                                    password="password",
                                                    database="plants")
